    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-4 py-20">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
          ...
          <h2 className="text-4xl font-black font-display tracking-tighter mb-3">Post Not Found</h2>
          <p className="text-foreground/40 mb-8">This blog post may have been removed or is not yet published.</p>
          <Link href="/blog"
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 pt-28 pb-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          {/* Back */}
          <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-bold text-foreground/40 hover:text-primary transition-colors mb-8">
          ...
          <motion.h1
            ...
            className="text-4xl sm:text-5xl font-black font-display tracking-tighter leading-tight mb-6"
          >
            {post.title}
          </motion.h1>
          ...
          <motion.div
            ...
            className="flex items-center gap-4 text-sm text-foreground/40 pb-8 mb-8 border-b border-white/5 flex-wrap"
          >
          ...
          {post.excerpt && (
            <motion.p
              ...
              className="text-xl text-foreground/60 leading-relaxed mb-8 font-medium italic border-l-2 border-primary pl-6"
            >
              {post.excerpt}
            </motion.p>
          )}
          ...
          >
            {paragraphs.map((para, i) => {
              if (para.startsWith('# ')) {
                return <h2 key={i} className="text-3xl font-black font-display tracking-tighter mt-10 mb-4">{para.slice(2)}</h2>;
              }
              if (para.startsWith('## ')) {
                return <h3 key={i} className="text-2xl font-black font-display tracking-tight mt-8 mb-3">{para.slice(3)}</h3>;
              }
              if (para.startsWith('### ')) {
                return <h4 key={i} className="text-xl font-black mt-6 mb-2">{para.slice(4)}</h4>;
              }
              if (para.startsWith('- ')) {
                return (
                  <ul key={i} className="list-disc pl-6 mb-4">
                    <li className="text-foreground/65 leading-relaxed">{para.slice(2)}</li>
                  </ul>
                );
              }
              return (
                <p key={i} className="text-foreground/65 leading-relaxed mb-4 text-base">
                  {para}
                </p>
              );
            })}
