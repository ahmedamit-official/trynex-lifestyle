...
export default function AdminProducts() {
  ...
  const openModal = () => {
    reset({ featured: false, customizable: true, stock: 0 });
    setModalOpen(true);
  };
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      await deleteProduct({ id, request: { headers: getAuthHeaders() } });
      queryClient.invalidateQueries({ queryKey: getListProductsQueryKey() });
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        ...
        <button
          onClick={openModal}
          className="btn-glow flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-white text-sm shrink-0"
          style={{ background: 'linear-gradient(135deg, hsl(var(--primary)), rgba(255,152,64,1))', boxShadow: '0 6px 24px rgba(255,107,43,0.35)' }}
        >
          ...
          {filteredProducts.map((product, i) => (
            ...
            >
              ...
              <div className="aspect-square relative overflow-hidden" style={{ background: '#f9fafb' }}>
                ...
                {/* Low stock warning */}
                {product.stock <= 5 && (
                  <div className="absolute top-2 left-2 px-2 py-1 rounded-lg text-[10px] font-black flex items-center gap-1"
                    style={{ background: 'rgba(245,158,11,0.2)', border: '1px solid rgba(245,158,11,0.3)', color: '#f59e0b' }}>
                    <AlertTriangle className="w-3 h-3" /> Low Stock
                  </div>
                )}
                {product.featured && (
                  <div className="absolute top-2 right-2 px-2 py-1 rounded-lg text-[10px] font-black luxury-badge flex items-center gap-1">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className={`text-[11px] font-bold px-2.5 py-1 rounded-lg ${product.stock <= 5 ? '' : ''}`}
                    style={{
                      background: product.stock <= 5 ? 'rgba(245,158,11,0.1)' : product.stock === 0 ? 'rgba(239,68,68,0.1)' : 'hsl(0 0% 10%)',
                      color: product.stock === 0 ? '#f87171' : product.stock <= 5 ? '#f59e0b' : 'rgba(255,255,255,0.35)',
                      border: `1px solid ${product.stock === 0 ? 'rgba(239,68,68,0.2)' : product.stock <= 5 ? 'rgba(245,158,11,0.2)' : 'transparent'}`
                    }}>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-7 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input {...register("slug")} className={inputClass} style={inputStyle} placeholder="e.g. premium-hoodie" />
                  ...
                  <div className="col-span-2">
                    ...
                    <input {...register("imageUrl")} className={inputClass} style={inputStyle} placeholder="https://images.unsplash.com/..." />
                  </div>
                  <div className="col-span-2">
                  ...
                  <div className="flex items-center gap-3">
                    <input type="checkbox" {...register("featured")} id="featured" className="w-4 h-4 rounded accent-primary" />
                    <label htmlFor="featured" className="text-sm font-semibold text-foreground/60 cursor-pointer">Mark as Featured</label>
                  </div>
                  <div className="flex items-center gap-3">
                    <input type="checkbox" {...register("customizable")} id="customizable" className="w-4 h-4 rounded accent-primary" />
                    <label htmlFor="customizable" className="text-sm font-semibold text-foreground/60 cursor-pointer">Allow Customization</label>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={isCreating}
                  className="btn-glow w-full py-4 mt-2 rounded-xl font-bold text-white flex items-center justify-center gap-2 disabled:opacity-50 disabled:transform-none"
                  style={{ background: 'linear-gradient(135deg, hsl(var(--primary)), rgba(255,152,64,1))', boxShadow: '0 6px 24px rgba(255,107,43,0.35)' }}
                >
                  <Plus className="w-4 h-4" />
                  {isCreating ? "Saving Product..." : "Add Product"}
                </button>
              </form>
            </motion.div>
          </motion.div>
