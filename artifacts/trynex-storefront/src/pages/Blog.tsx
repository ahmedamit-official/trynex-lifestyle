        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {allTags.length > 0 && (
              <div className="flex gap-2 flex-wrap items-center">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${!activeTag ? 'text-white bg-orange-500' : 'text-gray-500 hover:text-gray-700 bg-white'}`}
                  style={!activeTag ? {} : { border: '1px solid #e5e7eb' }}
                >
                  All
                </button>
                {allTags.map(tag => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTag === tag ? 'text-white bg-orange-500' : 'text-gray-500 hover:text-gray-700 bg-white'}`}
                    style={activeTag === tag ? {} : { border: '1px solid #e5e7eb' }}
                  >
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1,2,3,4,5,6].map(i => (
                <div key={i} className="rounded-3xl overflow-hidden animate-pulse bg-white border border-gray-100"
                  style={{ border: '1px solid #e5e7eb' }}>
                  <div className="aspect-video" style={{ background: '#f3f4f6' }} />
                  <div className="p-6 space-y-3">
                    <div className="h-4 rounded" style={{ background: '#e5e7eb', width: '60%' }} />
                    <div className="h-6 rounded" style={{ background: '#e5e7eb' }} />
                    <div className="h-4 rounded" style={{ background: '#e5e7eb', width: '80%' }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              {filtered.map((post, idx) => (
                ...
                >
                  <Link href={`/blog/${post.slug}`} className="block group">
                    <div className="rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-2 bg-white"
                      style={{
                        ...
                        {post.tags?.length > 0 && (
                          <div className="flex gap-2 mb-3 flex-wrap">
                            {post.tags.slice(0, 3).map(tag => (
                              <span key={tag} className="px-2 py-1 rounded-lg text-[10px] font-black text-gray-500"
